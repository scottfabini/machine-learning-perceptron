...
def main(argv=None):
    ...
    learning_rate = 0.1
    momentum = 0.9
    classifier = Classifier(image_size, learning_rate, momentum)
    print(classifier.classifier_output(training_images[0]))

    layer_size = 10
    layer = Layer(layer_size, image_size, learning_rate, momentum)
    print(layer.vector(training_images[0]))
    print(layer.prediction(training_images[0]))

    input_layer_size = image_size
    hidden_layer_size = 20
    output_layer_size = 10

    network = Network(input_layer_size, hidden_layer_size, output_layer_size, learning_rate, momentum)
    print(network.vector(training_images[0]))
    print(network.classification(training_images[0]))


# used with test data
    def output(self, inputs_):
        vector = self.output_vector(inputs_)
        numeric_prediction = np.argmax(vector).item()
